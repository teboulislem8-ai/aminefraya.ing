    ...
    return (
      <div style={B}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: '.6rem' }}>
          ...
          <div>
            <div style={{ fontSize: 12, fontWeight: 600 }}>أمين فرايا</div>
            <div style={{ fontSize: 10, color: '#9CA89C' }}>{new Date(p.created_at).toLocaleString('ar-DZ')}</div>
          </div>
        ...
        {p.media_url && p.type === 'file' && (
          <a href={p.media_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: '#185FA5', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
